...
class UILabel : UIView {
    ...
    override Vector2 calcMinSize(Vector2 parentSize) {
        ...
        if(fnt && multiline) {
            ...
            BrokenLines bl = fnt.breakLines(text, width / fontScale.x);
    ...
    override void layout(Vector2 parentScale, double parentAlpha) {
        ...
        if(multiline && fnt) {
            lines = fnt.breakLines(text, (frame.size.x / fontScale.x) + 0.01); // +0.01 is a hack to fix rounding differences between getMinSize and layout()
                    ...
                    if(drawCursor && x == cursorY) {
                        ...
                        for(uint i = 0; i <= line.length();) {
                            if(i == cursorX || i >= line.length()) {
                                int cursorOffset = (cursorX >= 0 && uint(cursorX) >= line.length() && cursorY == lines.count() - 1 ? - 3 : (fnt.StringWidth(cursorString) * 0.5) * fontScale.x);  // Offset cursor if at end of line, or between characters
