...
class UIMenu : GenericMenu {
	...
	UIControl activeControl, lastActiveControl, dragControl;
	UIRecycler recycler;
	...
	Map<String, ViewFunctionHolder > viewInterfaceCallbacks;
	...
	void registerViewInterfaceCallback(string name, UIView receiver, Function<ui bool(UIView, int, int, int, bool)> callback) {
		...
		viewInterfaceCallbacks.insert(name, holder);
	}

	void unRegisterViewInterfaceCallback(string name) {
	...
	override void init(Menu parent) {
		...
		lastScreenSize = (Screen.GetWidth(), Screen.GetHeight());
		...
		calcScale(lastScreenSize.x, lastScreenSize.y);
	}
	...
	virtual bool handleInterfaceEvent(ConsoleEvent e) {
		...
		let funcHolder = viewInterfaceCallbacks.GetIfExists(e.name);
		if(funcHolder) {
			if(funcHolder.receiver) {
				...
				viewInterfaceCallbacks.remove(e.name);
			}
		}
	...
	virtual bool mouseUpEvent(ViewEvent ev) {
		...
		if(developer > 1) {
			Console.Printf("Mouse Up: x: %f  y: %f", ev.mouseX, ev.mouseY);
		}

		// Call mouse up on the previously selected Down view
		if(mDownView) {
			Vector2 mousePos = (ev.mouseX, ev.mouseY);
			mDownView.onMouseUp(mousePos);
			mDownView = null;
			return true;
		}
	...
	virtual void startDragging(UIControl source) {
		dragControl = source;
	}


	virtual void stopDragging(UIControl source) {
		dragControl = null;
	}
	...
	double getRenderTime() {
		// System.GetTimeFrac() is not working, so until I get that to work we will stick to MSTime()
		// Nevermind, I have fixed GetTimeFrac() in the engine
